...
int getCountUtil(int *st, int s, int e, int l, int r, int index)
{
    ...
    // ...s..[l..r]..e... overlap
    int val1 = getCountUtil(st, s, mid, l, r, index*2+1);
    int val2 = getCountUtil(st, mid+1, e, l, r, index*2+2);
    
    return val1 + val2;
}

int getCount(int *st, int n, int l, int r)
{
    if(l<0 || r<0 || l>=n || r>=n || l>r) return 0;
    
    return getCountUtil(st, 0, n-1, l, r, 0);
}
...
int main() {
	...
	// Queries for elements having values within the range X to Y in the given index range using Segment Tree
	// https://ide.geeksforgeeks.org/BWULOSe2KR
	// https://www.geeksforgeeks.org/queries-for-elements-having-values-within-the-range-a-to-b-in-the-given-index-range-using-segment-tree/?ref=rp
	...
	cout << "find count from index 2 to 6: " << getCount(st, n, 2, 6) << endl;
